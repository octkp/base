...
-- 環境: cuba7
-- 備考: 課金データを2つ作成（1年目と2年目）

-------------------------------
-- 変数設定
-------------------------------
SET v.company_id = '123588971';
...
-- 2年目の契約期間
SET v.charge_start_at = '2024-01-01';           -- 課金開始日（1年目開始）
SET v.charge_period_start_date = '2025-01-01';  -- 契約期間開始日（2年目）
...
-- 2-1. charge_historiesクリア（2024年2月以降の全課金）
DELETE FROM charge_histories
WHERE charge_histories.ch_charge_id IN (
    SELECT charges.charge_id FROM charges
    WHERE charges.charge_company_id = current_setting('v.company_id')::int
      AND charges.charge_date >= '2024-02-01'::date
);

-- 2-2. chargesクリア（2024年2月以降の全課金）
DELETE FROM charges
WHERE charges.charge_company_id = current_setting('v.company_id')::int
  AND charges.charge_date >= '2024-02-01'::date;

-- 2-3. charges作成（1年目: 2024-02-01〜2025-01-31）
INSERT INTO charges (
    charge_company_id,
    charge_env,
    charge_status,
...
FROM charges
WHERE charges.charge_company_id = current_setting('v.company_id')::int
  AND charges.charge_year = 2024
  AND charges.charge_month = 2;

-- 2-5. charges作成（2年目: 2025-02-01〜2026-01-15）
INSERT INTO charges (
    charge_company_id,
    charge_env,
    charge_status,
