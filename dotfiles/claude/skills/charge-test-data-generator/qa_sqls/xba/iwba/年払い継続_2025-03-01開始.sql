-- 目的: 年払い→年払い継続パターン（契約期間: 2025-03-01〜2026-02-28）
-- 検証目的: 年払い→年払いの更新確認
...
-- 変数設定
-------------------------------
SET v.company_ids = '123947865, 123947877, 123947857, 123947849, 123947869, 123947873, 123947861, 123947853';
...
-- 年払い契約期間
SET v.charge_start_at = '2025-03-01';           -- 課金開始日（月初のみ設定可能）
SET v.charge_period_start_date = '2025-03-01';  -- 契約期間開始日（課金開始日に合わせる）
SET v.charge_period_end_date = '2026-02-28';    -- 契約期間終了日（12ヶ月後の月末）
...
-- テストデータ更新
-------------------------------
-- 2-0. 既存chargesのcharge_envをtest1に統一
UPDATE charges
SET charge_env = 'test1'
WHERE charges.charge_company_id = ANY(
    string_to_array(regexp_replace(current_setting('v.company_ids'), '\s', '', 'g'), ',')::int[]
)
  AND charges.charge_env != 'test1';
...
SELECT
    unnest(string_to_array(regexp_replace(current_setting('v.company_ids'), '\s', '', 'g'), ',')::int[]),
    'test1',
    'OK',
    'ANNUAL',
...
WHERE companies.company_id = ANY(
    string_to_array(regexp_replace(current_setting('v.company_ids'), '\s', '', 'g'), ',')::int[]
)
RETURNING
    *;
