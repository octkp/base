-- SQL No: 10
-- パターン: K-02
-- 目的: 年払い満了→月払い変更＋課金開始日が未来日の状態を作成
-- 検証目的: 契約終了後の無料期間（再無料期間）発生時の挙動確認
...
-- 変数設定
-------------------------------
SET v.company_id = '123560475';

-- 年払い→月払い変更の課金設定（課金開始日は未来日）
...
-- 年払い契約期間、課金開始日は未来日
SET v.charge_start_at = '2026-02-01';           -- 課金開始日（未来日）
SET v.charge_period_start_date = '2025-01-01';  -- 契約期間開始日
...
-- 2-1. charge_historiesクリア（契約期間開始日以降の全レコード）
DELETE FROM charge_histories
WHERE charge_histories.ch_charge_id IN (
    SELECT charges.charge_id FROM charges
    WHERE charges.charge_company_id = current_setting('v.company_id')::int
      AND charges.charge_date >= current_setting('v.charge_period_start_date')::date
);

-- 2-2. chargesクリア（契約期間開始日以降の全レコード）
DELETE FROM charges
WHERE charges.charge_company_id = current_setting('v.company_id')::int
  AND charges.charge_date >= current_setting('v.charge_period_start_date')::date;

-- 2-3. charges作成（年払い1年分、過去の課金として）
INSERT INTO charges (
    charge_company_id,
    charge_env,
    charge_status,
