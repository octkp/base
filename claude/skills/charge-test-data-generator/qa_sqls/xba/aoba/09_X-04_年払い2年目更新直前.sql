...
-- 2-1. charge_historiesクリア（2024年1月以降の全課金）
DELETE FROM charge_histories
WHERE charge_histories.ch_charge_id IN (
    SELECT charges.charge_id FROM charges
    WHERE charges.charge_company_id = current_setting('v.company_id')::int
      AND charges.charge_date >= '2024-01-01'::date
);

-- 2-2. chargesクリア（2024年1月以降の全課金）
DELETE FROM charges
WHERE charges.charge_company_id = current_setting('v.company_id')::int
  AND charges.charge_date >= '2024-01-01'::date;

-- 2-3. charges作成（1年目: 2024-01-01〜2024-12-31）
INSERT INTO charges (
    charge_company_id,
    charge_env,
    charge_status,
...
-- 2-5. charges作成（2年目: 2025-01-01〜2025-12-31）
INSERT INTO charges (
    charge_company_id,
    charge_env,
    charge_status,
