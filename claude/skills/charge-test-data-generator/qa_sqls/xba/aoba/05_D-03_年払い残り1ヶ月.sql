...
-- 契約期間中（残り1ヶ月）
-- 契約開始を11ヶ月前に設定し、残り1ヶ月の状態を作成
SET v.charge_start_at = '2025-02-01';           -- 課金開始日（月初のみ設定可能）
SET v.charge_period_start_date = '2025-02-01';  -- 契約期間開始日
SET v.charge_period_end_date = '2026-01-31';    -- 契約期間終了日（検証月末）
SET v.is_charge_target = '1';                   -- 課金対象
SET v.unsubscribe_scheduled_at = '2026-01-31 20:00';  -- 退会予約日（契約満了日）
...
SET
    ...
    company_unsubscribe_scheduled_at = current_setting('v.unsubscribe_scheduled_at')::timestamp
WHERE company_id = current_setting('v.company_id')::int
RETURNING
    *;
