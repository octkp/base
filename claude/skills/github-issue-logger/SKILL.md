...
例:
- Issue: "Add dark mode support" → `~/dotfiles/tasks/Add-dark-mode-support/`
- Issue: "Fix: login bug on Safari" → `~/dotfiles/tasks/Fix_-login-bug-on-Safari/`

### 4. ディレクトリ作成

```bash
mkdir -p ~/dotfiles/tasks/{issueタイトル}
```
...
2. `~/dotfiles/tasks/{issueタイトル}/` ディレクトリ作成
